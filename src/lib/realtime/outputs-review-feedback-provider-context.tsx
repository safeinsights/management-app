...
// Mirror of code-review-feedback-provider-context for the outputs-review-feedback document. A
// distinct context per document name is required: HocuspocusProviderWebsocket dispatches by
// document name.
    ...
    const [state] = useState<OutputsReviewFeedbackProviderShareState>(() => {
        ...
        return {
            ...
            subscribe: (notify) => {
                ...
                // The editor publishes from a child effect, and React runs those before the parent
                // effect that subscribes here, so a late subscriber has to be handed what it missed
                // or it waits for an edge that has already passed.
                notify(current)
                return () => {
